...
// Register ingest as a global Alpine store so it's accessible from any
// nested x-data scope via $store.ingest (fixes ReferenceError on nav button)
...
function sectorApp() {
  return {
    ...
    // Keep local ingest alias for backwards compat with any x-text bindings on body scope
    get ingest() { return Alpine.store('ingest'); },
    ...
    // ── Data refresh — uses $store.ingest for cross-scope access ─────────────
    async triggerRefresh(sic) {
      const store = Alpine.store('ingest');
      ...
      try {
        ...
        const data = await resp.json();
        if (!resp.ok) { alert(data.error || 'Refresh failed'); store.running = false; return; }
        store.jobId = data.jobId;
        this.pollIngestStatus();
      } catch (e) {
        alert('Network error — could not start refresh');
        store.running = false;
      }
    },
    ...
    pollIngestStatus() {
      ...
      store.pollTimer = setInterval(async () => {
        try {
          const resp = await fetch('/api/ingest/status/' + store.jobId);
          const data = await resp.json();
          const done  = data.adapters_total || 1;
          const compl = data.adapters_done  || 0;
          store.progress = Math.round((compl / done) * 100);
          if (data.status === 'complete' || data.status === 'failed') {
            clearInterval(store.pollTimer);
            store.running  = false;
            store.progress = 100;
            if (data.status === 'complete') window.location.reload();
            else alert('Refresh failed — check logs for details.');
          }
